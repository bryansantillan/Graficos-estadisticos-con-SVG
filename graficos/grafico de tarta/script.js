...
function generarGrafico(event) {
  ...
  if (isNaN(valor1) || isNaN(valor2) || isNaN(valor3) || isNaN(valor4) || isNaN(valor5) ||
    ...
    //Dibujar leyenda
    //Array de colores con los que se pintará la leyenda
    ...
    const leyenda = [];
    //Coordenadas de inicio para pintar la leyenda
    coordX = 20;
    coordY = 600;

    //Agregar los datos al array leyenda y actualizar coordY
    porciones.forEach((porcion, index) => {
      if (valores[index] > 0) {
        coordY = coordY + 20;
        const seccion = index + 1;
        // console.log(seccion);
        const frase = `Porción ${seccion}: ${porcion}%`;
        leyenda.push({ x: 50, y: coordY, content: frase, color: seccion });
      }
    });

    //Crear el cuadro de la leyenda
    const rectElement = document.createElementNS("http://www.w3.org/2000/svg", "rect");
    rectElement.setAttribute("x", 40);
    rectElement.setAttribute("y", 600);
    rectElement.setAttribute("width", 165);
    rectElement.setAttribute("height", coordY + 15 - 600); //Altura según los elementos que haya
    rectElement.setAttribute("rx", 10); //Redondear bordes
    ...
    leyenda.forEach((data, index) => {
      ...
      //Escoger color del array Colores según el número asignado en data.color-1
      const color = colores[data.color - 1];
      //Se asigna el color al elemento de texto
